...
return {
	...
	config = function()
		-- set keymaps
		-- Define a function to save and compile
		local function save_and_compile()
			-- Save the current file
			vim.cmd(":w")
			-- Execute xelatex command
			vim.cmd(":!xelatex %")
		end

		local keymap = vim.keymap -- for conciseness
		keymap.set("n", "<leader>cm", ":w<CR>:!xelatex -shell-escape -file-line-error -halt-on-error **.tex %<CR>", { desc = "save and compile" })
		-- Latex PDF shortcut
		keymap.set("n", "<leader>vv", "<plug>(vimtex-view)", { noremap = true, silent = true })
